...
export default async function SwimPage() {
  const products = await getProductsByCategory('swim');

  return (
    <main className="flex min-h-screen flex-col w-full bg-background pt-40">
      <div className="container mx-auto px-6 text-center max-w-4xl">
        <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight uppercase">Swimwear</h1>
        <p className="text-xl text-muted-foreground font-medium">
          Make a splash. Resort-ready styles for the modern queen.
        </p>
      </div>
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 px-6 text-center">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            Our latest swimwear collection is currently being prepared. Check back shortly for our new drops.
          </p>
        </div>
      ) : (
